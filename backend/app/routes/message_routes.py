...
def get_conversation_messages(conversation_id):
    ...
    page = request.args.get('page', 1, type=int)
    per_page = request.args.get('per_page', 20, type=int) # 20 par défaut pour une conversation
    skip = (page - 1) * per_page

    # Filtrer par conversation_id ET (l'utilisateur est expéditeur OU l'utilisateur est dans les destinataires)
    query = {
    ...
    # Trier par date croissante (le plus vieux au plus récent) pour une vue type chat
    messages_cursor = mongo.db.messages.find(query).sort("date_envoi", 1).skip(skip).limit(per_page)
    ...
    for msg in messages_cursor:
        ...
        # Statut de lecture spécifique à l'utilisateur courant (si pertinent)
        user_read_status = next((s for s in msg.get('statut_lecture', []) if s.get('utilisateur_id') == user_obj_id), None)
        msg['mon_statut_lecture'] = user_read_status if user_read_status else {"lu": False, "date_lecture": None}
